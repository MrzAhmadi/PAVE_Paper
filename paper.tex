...
\begin{figure}[htbp]
    ...
    \caption{PAVE pipeline output overview: overall configuration availability (left) and exit-node type distribution among working configurations (right), across 109,874 tested records.}
...
On top of this platform, the PAVE implements a specification inference engine that takes proxy URIs as input and emits the corresponding connectivity results as output.
...
After the downloaded configuration list occupied about \textbf{5 MB} on the host machine storage, a \textbf{30-second} test by PAVE was run on all target configurations to determine which configurations are reachable by PAVE, and \textbf{874} were confirmed at the end.
...
Generally \textbf{109,874} records about the entire connectivity testing process are recorded by the PAVE pipeline.
