...
\title{An Empirical Study on \\Automated Free Proxy Configuration Testing \\and Security Analysis from Public Subscription Feeds}
...
We design and implement the Config Probe System (CPS), a containerized, resource-conscious dynamic analysis infrastructure used to orchestrate, test, and segment proxy configurations across a massive subscription dataset.
...
Our empirical evaluation exposes 874 reachable configurations across five active proxy protocols, out of 109,874 deduplicated configuration records.
We benchmark five distinct proxy protocols (VLESS, VMess, Shadowsocks, Trojan, and Hysteria2) across 109,874 evaluation records.
...
To address missing or unreachable configurations, researchers employ automated connectivity testing, a technique that probes structural proxy definitions from runtime network behavior.
...
To execute this study under uniform testing constraints, we design and implement the Config Probe System (CPS).
CPS serves as a containerized, resource-conscious dynamic analysis infrastructure used to orchestrate configuration fetching, test connectivity, and segment multi-protocol results across a massive target dataset.
...
\begin{LaTeXdescription}
    \item[Quantifying Configuration Scarcity] Systematically identifying unreachable endpoints and structural mutations to illuminate the real-world severity of proxy and configuration staleness in the wild.
    \item[Benchmarking State-of-the-Art Protocols] Evaluating reachability rates, latency characteristics, and exit-node security profiles of distinct proxy protocols (VLESS~\cite{xray_core}, VMess~\cite{v2ray}, Shadowsocks~\cite{shadowsocks}, Trojan~\cite{trojan_gfw}, and Hysteria2~\cite{hysteria2_proto}) across identical, large-scale subscription records.
    \item[Identifying Infrastructure Bottlenecks] Measuring how runtime constraints, security controls (e.g., SSL handshake failures), and automated connectivity parameterization impact automated black-box parallel workflows at scale.
...
Our empirical evaluation exposes 874 reachable configurations across five active protocols, out of 109,874 deduplicated evaluation records.
...
\begin{LaTeXdescription}
    ...
    \item[Scalable Architecture] Implementation of a scalable system architecture that leverages dynamic resource allocation and parallel execution to reduce the overall testing time significantly.
    \item[Advanced Data Deduplication] Development of a multi-stage noise reduction mechanism that strictly filters duplicate and expired configurations, transforming raw subscription data into actionable information.
    \item[Experimental Repeatability] Engineering a deterministic system infrastructure that guarantees the repeatability of the experiments at every stage of the pipeline.
    ...
    \item[Distinguishing Results] Introducing multiple categories of configurations, such as unreachable, datacenter-hosted, and residentially-routed proxies, with security profiles including DNS leak and IPv6 leak detection.
\end{LaTeXdescription}
\end{sloppypar}
...
The configprobe toolset provides a comprehensive suite of features for implementing this proxy testing architecture.
...
\begin{figure}[htbp]
    ...
    \caption{CPS pipeline output overview: overall configuration availability (left) and exit-node type distribution among working configurations (right), across 109,874 tested records.}
...
Hysteria2~\cite{hysteria2_proto} is a modern UDP-based protocol client that supports the Hysteria2 and TUIC~\cite{tuic} protocols, offering low-latency connectivity and built-in congestion control.
Sing-box~\cite{singbox} is a universal proxy platform that supports TUIC and ShadowsocksR~\cite{shadowsocksr} configurations and is designed to be easily integrated into automated testing pipelines.
...
We present now our Config Probe System (CPS).
...
Multiple tests can run in parallel, but all experiments should be monitored by the resources observer to ensure we do not exceed the machine's resources.
...
In this approach, we have two options for running experiments: either running all processes in a single container and managing multiple test workers within it, or using a base image with all configurations set up to run multiple containers, with the proxy clients running in each container.
We have chosen the first approach because it involves less interaction with the container management layer, which is less problematic.
...
The script is not just responsible for running the containers, but also for finishing each container after it completes its job; furthermore, it monitors the remaining resources to ensure we do not run out by running many containers in parallel, and finally the script could be the transporter of needed parameters such as the number of shards, the shard index, the worker count, and the output directory inside each container.
...
In this image, we provision an Ubuntu-based environment and install essential dependencies, such as the Xray core~\cite{xray_core}, Hysteria2 client~\cite{hysteria2_proto}, Sing-box~\cite{singbox}, and Python~\cite{python_org}.

\begin{algorithm}[t]
\caption{Actor Script Orchestration and Resource Management}
\label{alg:actor}
\begin{algorithmic}[1]
    \State \textbf{Initialize} logging and directory structures
    \State \textbf{Define} resource thresholds $T_{RAM}, T_{CPU}$
    \State \textbf{Build} experiment base image $I_{exp}$
    \State \textbf{Fetch} all subscription URLs and deduplicate
    \State \textbf{Split} deduplicated configs into $N$ shards
    \For{each $Shard_i$ in $Shards$}
        \While{true}
            \State $R_{avail} \leftarrow$ \text{CheckAvailableResources()}
            \If{$R_{avail}.RAM > T_{RAM}$ \textbf{and} $R_{avail}.CPU > T_{CPU}$}
                \State \textbf{Run} container $C_i$ with shard parameters
                \State \textbf{Mount} persistent volumes for configs and results
                \State \textbf{Start} container log stream
                \State \textbf{Wait} \textit{staggered\_boot\_delay}
                \State \textbf{Break}
            \Else
                \State \textbf{Wait} \textit{resource\_retry\_delay}
            \EndIf
        \EndWhile
    \EndFor
\end{algorithmic}
...
\subsection{Dynamic Resource Orchestration and Monitoring}
One of the most important parts of the experiment that makes it significantly faster is the parallel execution of testing workers, which requires a good approach to manage worker instances to run in parallel, considering resources that, in this experiment, are focused on the processor and memory of the system.
The script (Algorithm~\ref{alg:actor}) tries to set limits on memory usage and processes per container, which depend heavily on the number of concurrent workers and the protocol under test; supporting a large portion of configurations requires newer, more resource-intensive proxy clients, and newer clients require more resources.
Furthermore, the script would be programmable with two parameters: a delay after running each container before running the next, and a delay before checking the available resources.
Using this approach, the script considers available resources after a delay before running.
If there are enough free resources for a container, it runs a new container; otherwise, it delays checking for a duration specified by the resource checker parameter.
Finally, to ensure stability of the host machine, another parameter will be available to prevent the entire running experiment from occupying all resources.

\subsection{Intra-Container Lifecycle}
After containerizing the testing environment, the automated system, which is a script (Algorithm~\ref{alg:container}), tries to run a container for each configuration shard.
Within the container, the system should first verify the shard index and configuration count to prevent future problems if the parameters passed as arguments are incorrect.
...
A dedicated test session is responsible for running DNS leak checks, IPv6 leak detection~\cite{vpn_survey2023}, and proxy detection queries through each live proxy port.
...
These configurations are present in multiple subscription feeds and are visible in every experiment and captured dataset.
Multiple channels distribute the same configuration, and even when some servers are not reachable, these kinds of duplicates are present in the captured data.
...
It acts as a detailed, chronological log of the conversation between the CPS client and a remote exit server.
...
The main objective is to define the tools and compare them, and then to define the scoring and metrics system for evaluating the CPS system.
...
Although this tool is based on Go, it supports a robust command-line interface (CLI) integration and is highly practical for use with CPS, which is developed in Python.
...
For instance, converting \texttt{vless://uuid@server:443?type=ws\&path=/api} into a running Xray process with the correct WebSocket transport.
...
This Go-based tool has a straightforward CLI and is designed to be easily integrated into the CPS pipeline.
...
It supports sensitive data handling, which means that by using the \texttt{--config} command-line flag, it will parse the actual captured authentication tokens.
Meanwhile, there is a bottleneck in this tool: unlike fully synchronous tools, this tool requires the orchestrator to poll for readiness before accepting SOCKS5 connections, and there is no callback or event notification system.
...
On top of this platform, the CPS implements a specification inference engine that takes proxy URIs as input and emits the corresponding connectivity results as output.
Additionally, the same tool is capable of computing the connection attempt outcome for a configuration, possibly measuring the difference between nominal and actual behavior of the server.
...
These tools are integrated outside the CPS pipeline, as they are used for post-processing and analysis of the generated connectivity records.
...
\textbf{Protocols} & VLESS/VMess/SS/Trojan~\cite{xray_core,v2ray,shadowsocks,trojan_gfw} & Hysteria2/TUIC~\cite{hysteria2_proto,tuic} & TUIC/SSR~\cite{tuic,shadowsocksr} \\ \hline
\textbf{CLI} & High (1-pass) & Med (poll-ready) & High (JSON-cfg) \\ \hline
...
In this section, how to prepare required datasets to run and evaluate CPS is discussed.
Fundamentally, CPS requires a dataset of proxy configurations and after ending the pipeline of the test, it is required to compare generated results with the expected ground truth.

\subsection{Ground Truth Definition}
Evaluation of the CPS outputs demands preparing specific information to compare.
...
\begin{itemize}
    \item \textbf{Unreachable:} Configurations that CPS could not connect through.
...
It is expected to run CPS first, then investigate for corresponding reachability results, but after many efforts in this project, the experiment demonstrates that random selection of candidate subscription feeds does not guarantee availability of reachable proxy endpoints.
For this reason, before running the CPS, it is necessary to investigate target candidate subscription sources.
...
\texttt{barry-far/V2ray-config}~\cite{barry_far_v2ray} is a comprehensive repository of proxy configurations collected from various public subscription channels, including protocol-separated feeds and combined subscription lists, maintained by a community contributor.
Within this project, this repository is used as a primary source for sourcing proxy configuration URIs across VLESS, VMess, Trojan, Shadowsocks, and ShadowsocksR~\cite{shadowsocksr} protocols.
\texttt{ebrasha/free-v2ray-public-list}~\cite{ebrasha_v2ray} is a comprehensive repository of free proxy configurations collected from various sources and maintained by EbraSha.
Within this project, this repository is used as a supplementary source for sourcing additional multi-protocol proxy configuration URIs corresponding to the candidate subscription feeds.
...
This section presents the results obtained from executing the CPS pipeline.
...
The result of running the connectivity testing system on the collected subscription dataset produced \textbf{109,874} unique deduplicated configuration records, of which \textbf{874} were reachable at the end.
After the downloaded configuration list occupied about \textbf{5 MB} on the host machine storage, a \textbf{30-second} test by CPS was run on all target configurations to determine which configurations are reachable by CPS, and \textbf{874} were confirmed at the end.
By running \textbf{50} concurrent workers on the host machine simultaneously, the entire process took several hours to complete.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\columnwidth]{protocol_breakdown.png}
    \caption{Protocol breakdown: total configuration count vs.\ reachable count per protocol (left) and availability rate per protocol (right). Hysteria2 achieves the highest availability rate at 23.0\%.}
...
\hline
\textbf{VLESS}      & 80,470 & 600  & 0.75\% \\
\textbf{Trojan}     & 18,416 & 113  & 0.61\% \\
\textbf{SS}         & 7,346  & 62   & 0.84\% \\
\textbf{VMess}      & 3,478  & 64   & 1.84\% \\
\textbf{Hysteria2}  & 152    & 35   & 23.0\% \\
\textbf{SSR}        & 1      & 0    & 0.00\% \\
\textbf{TUIC}       & 5      & 0    & 0.00\% \\
\textbf{SOCKS}      & 6      & 0    & 0.00\% \\
\hline
\textbf{Total}      & 109,874 & 874 & 0.80\% \\
\hline
\end{tabular}
\end{table}
...
Generally \textbf{109,874} records about the entire connectivity testing process are recorded by the CPS pipeline.
...
\hline
Host unreachable             & 45,071 \\
Timeout                      & 32,880 \\
SSL handshake failed         & 16,018 \\
xray failed to start         & 14,999 \\
hysteria2 failed to start    &     15 \\
curl error 60                &      6 \\
curl error 18                &      5 \\
SOCKS proxy error            &      4 \\
sing-box failed to start     &      1 \\
curl error 97                &      1 \\
\hline
...
Datacenter exit   & 592 & 67.7\% & -- \\ \hline
Residential exit  & 282 & 32.3\% & -- \\ \hline
Blacklisted IP    & 496 & 56.8\% & -- \\ \hline
Proxy-detected    & 445 & 50.9\% & -- \\ \hline
\end{tabular}
\end{table}
...
It has found about 35 reachable endpoints in a small dataset of 152 configurations, yielding a striking 23.0\% availability rate.
Then VMess with 64 reachable from 3,478 has a very similar behavior with SS at 1.84\% and 0.84\%, respectively.
At the end, VLESS has the most entries in the dataset but also the most fragile reachability in comparison to the other protocols.

In the geographic distribution of reachable configurations, the results are significantly different.
Germany is the most represented exit country with 224 working configurations, with a large gap with respect to other countries.
The second exit country was France with 84 reachable configurations and an acceptable result.
The Netherlands, the United States, and Singapore follow with 77, 76, and 52 respectively.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\columnwidth]{geo_distribution.png}
    \caption{Top 20 exit countries for reachable configurations. Germany (224) and France (84) account for the largest share of working proxy exit nodes.}
...
\textbf{1}  & Germany            & 224 \\
\textbf{2}  & France             &  84 \\
\textbf{3}  & The Netherlands    &  77 \\
\textbf{4}  & United States      &  76 \\
\textbf{5}  & Singapore          &  52 \\
\textbf{6}  & Poland             &  40 \\
\textbf{7}  & Finland            &  36 \\
\textbf{8}  & Japan              &  36 \\
\textbf{9}  & Italy              &  29 \\
...
The results of the experiment demonstrate the most unstable connectivity scenario arises from SSL handshake failures, which account for 16,018 records and indicate server-side certificate pinning or TLS version mismatches.
...
\begin{figure}[htbp]
    ...
    \caption{Latency distribution across all reachable configurations (left, capped at 5000\,ms; median 1472\,ms, mean 1955\,ms) and latency by protocol as a box-plot (right). Hysteria2 shows the lowest median latency.}
...
The overall availability rate of 0.80\% across 109,874 configurations confirms that the free proxy ecosystem is characterized by severe configuration staleness, with the vast majority of published configurations pointing to offline or unreachable servers.
...
The results show VLESS and VMess take significantly more time to process with respect to Hysteria2, with a mean latency of 1,955 ms and a median latency of 1,472 ms across all working configurations.
...
By applying Random Forest, Gradient Boosting, Logistic Regression, and a Multi-Layer Perceptron (MLP) to predict configuration availability from static metadata (protocol, port, and port class), the best achievable ROC-AUC score is approximately 0.74.
...
\begin{figure}[htbp]
    ...
    \caption{Availability prediction model comparison by ROC-AUC. Gradient Boosting achieves the best score (0.783), followed by Random Forest (0.775), MLP (0.737), and Logistic Regression (0.689). All models outperform the random baseline (0.5).}
...
\hline
\textbf{1} & Random Forest       & 0.742 & -- \\
\textbf{2} & MLP (PyTorch)       & 0.738 & -- \\
\textbf{3} & Gradient Boosting   & 0.734 & -- \\
\textbf{4} & Logistic Regression & 0.620 & -- \\
\hline
\end{tabular}%
}
\end{table}
...
Some of the most important findings are related to CPS implementation.
The results of this project demonstrate that the implemented CPS for proxy configurations is a practical and effective approach for testing connectivity and generating reachability records.
By considering many important aspects of the implementation, such as containerization, automation, and scalability, the CPS is able to handle a wide range of configurations and test their connectivity in a reliable manner.
...
For future work, it would be interesting to extend the CPS to support capturing proxy connectivity that uses non-standard transport layers.
...
In this project, the UI exploration component of the CPS is based on a simple random testing strategy by the curl tool, which is a built-in tool in the Linux SDK.
...
In this project, the CPS is primarily focused on dynamic analysis of proxy connectivity.
At the end of the CPS results, it is observable that there are some configurations that CPS is not able to establish any proxy connectivity for, and it cannot cross their security barriers.
For future work, it would be interesting to explore the integration of the CPS with static analysis tools to break the security barriers of configurations, and especially TLS certificate pinning bypass.
